...
let AnalyticsService = class AnalyticsService {
    ...
    async overview(query) {
        ...
        const [users, activeUsers, orders, dailyOrders, subscriptions, leads, inventory, notifications] = await Promise.all([
    ...
    async orderWindowMetrics(range) {
        const now = new Date();
    ...
    async subscriptionMetrics(range) {
    ...
    async notificationMetrics(range) {
    ...
    resolveRange(query) {
        ...
        if (query.period === "monthly")
            from.setUTCMonth(to.getUTCMonth() - 1);
        return { from, to };
    }
    startOfDay(value) {
    ...
    flatten(payload) {
        if (Array.isArray(payload))
            return payload.flatMap((item) => this.flatten(item));
        ...
        for (const [key, value] of Object.entries(payload)) {
            if (Array.isArray(value)) {
                value.forEach((item, index) => {
                    for (const row of this.flatten(item))
                        rows.push({ group: key, index, ...row });
                });
                continue;
            }
            if (value && typeof value === "object" && !(value instanceof Date)) {
                for (const row of this.flatten(value))
                    rows.push({ group: key, ...row });
                continue;
            }
            rows.push({ key, value });
        }
